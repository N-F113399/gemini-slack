...
test("validateSlackFile rejects missing file id", () => {
  assert.throws(
    ...
    (error) => error instanceof ContentError && error.code === CONTENT_ERROR_CODES.INVALID_CONTENT,
  );
});
...
test("validateSlackFile rejects a file over the configured size limit", () => {
  ...
  try {
    assert.throws(
      ...
      (error) => error instanceof ContentError && error.code === CONTENT_ERROR_CODES.CONTENT_TOO_LARGE,
    );
  } finally {
