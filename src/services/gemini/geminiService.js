...
function backoffDelay(attemptIndex) {
  const exp = Math.min(
    BASE_BACKOFF_MS * 2 ** Math.max(attemptIndex - 1, 0),
    MAX_BACKOFF_MS,
  );
 ...
 *
 * This service owns model selection, retry/backoff, fallback models and
 * response extraction. It does not know about Slack or the database.
 */
export async function generate({ contents }) {
  ...
  const retryModels = getRetryModelNames().slice(0, retryLimit);
  ...
  for (let index = 0; index < modelCandidates.length; index += 1) {
    ...
    logger.info(
      `🔁 Gemini request attempt ${index + 1}/${modelCandidates.length} with model=${currentModel}`,
    );

    let res;
    let data;
    try {
      ({ res, data } = await generateContent({
        contents,
    ...
    logger.warn(
      `Gemini error (status=${res.status}) on model=${currentModel}: ${lastErrorMsg}. Retrying with next model.`,
    );
  }
