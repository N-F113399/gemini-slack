...
const FRESHNESS_PATTERNS = [
  ...
  /(最新情報|最新|現在|ニュース|最近|今週|今月|今年|今日|昨日)/,
  /(直近|最新の動向|最新の状況)/,
];

const RESEARCH_INTENT_PATTERNS = [
  /\b(?:search\s+for|look\s+up|research)\b/i,
  /\b(?:find\s+(?:information|out)|check\s+online)\b/i,
  /(調べて|検索して|探して|ウェブで確認して|オンラインで確認して|リサーチして)/,
];

const VOLATILE_INFORMATION_PATTERNS = [
  /\b(price|pricing|stock|share|exchange\s+rate|weather|schedule|availability)\b/i,
...
export function decideSearch(text = "") {
  ...
  if (explicitQuery) {
    return Object.freeze({ shouldSearch: true, reason: "explicit", query: explicitQuery });
  }

  if (FRESHNESS_PATTERNS.some(pattern => pattern.test(trimmed))) {
    return Object.freeze({ shouldSearch: true, reason: "freshness", query: trimmed });
  }

  if (VOLATILE_INFORMATION_PATTERNS.some(pattern => pattern.test(trimmed))) {
    return Object.freeze({ shouldSearch: true, reason: "volatile_information", query: trimmed });
  }

  if (RESEARCH_INTENT_PATTERNS.some(pattern => pattern.test(trimmed))) {
    return Object.freeze({ shouldSearch: true, reason: "research_intent", query: trimmed });
  }
